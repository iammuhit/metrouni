...
class Metrouni {
    ...
    public function __construct() {
        ...
        $this->_CI->lang->load('metrouni/metrouni');
    }
    ...
    public function info($info, $lang_file = NULL) {
        ...
        if ($info['backend'] === TRUE && $info['metrouni_core'] !== TRUE)
            return $info;

        // Lets check if the Metrouni core module has been loaded yet...
        if (!class_exists('Module_Metrouni'))
            return $info;

        // Add the Metrouni asset paths
        if (file_exists(SHARED_ADDONPATH . 'modules/metrouni/details.php')) {
            $dir = BASE_URL . 'addons/shared_addons/modules/';
        } else {
            $dir = BASE_URL . 'addons/' . SITE_REF . '/modules/';
        }

        Asset::add_path('metrouni', $dir . 'metrouni/');

        // Retrieve the MetroUNI sub-modules
        $this->_CI->db->where("(`is_backend` = 0 OR `slug` = 'metrouni')", NULL, FALSE)->order_by('CASE WHEN `slug`="metrouni" THEN 0 ELSE 1 END', NULL, FALSE)->order_by('id', 'asc');
        $subs = $this->_CI->db->get_where('modules', array('menu' => 'MetroUNI', 'installed' => '1', 'enabled' => '1'));
        ...
        if ($subs->num_rows()) {
            foreach ($subs->result() as $module) {
                ...
                if (class_exists($class)) {
                    ...
                    // Get this modules info (the MetroUNI way)
                    ...
                    /* if( substr($this->_CI->uri->segment(2), 0, 8) == 'metrouni' AND empty($_POST) )
                      {
                      ...
                      if( substr($this->_CI->uri->segment(2), 0, 8) == 'metrouni' && empty($_POST) )
                      {
    ...
    public function is_installed() {
        ...
        $this->_CI->session->set_flashdata('error', lang('metrouni:install:not_installed'));
...
/* End of file metrouni.php */
/* Location: ./modules/metrouni/libraries/metrouni.php */